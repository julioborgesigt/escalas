 ...
 * circulam fora dele. São cinco aqui, juntos porque compartilham a mesma
 * moldura e as mesmas medidas:
 *
 *   escala de FDS · expediente · plantão mensal · escala GISE ·
 *   relatório de produtividade GISE
 *
 * O relatório de serviço extraordinário saiu para `pdf-relatorio-extra.ts`: é a
 * única família com moldura PRÓPRIA — `iniciarRelatorioExtra`,
 * `tabelaPresencasRelatorio` e companhia não são usados por nenhuma escala. O
 * que de fato é comum aos dois arquivos (tipos da GISE, logos do topo, formato
 * de imagem) está em `pdf-comum.ts`.
 *
...
import { formatarData, formatarDataExtenso, MESES_PT } from '../../utils/datas';
