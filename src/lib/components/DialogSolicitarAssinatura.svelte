...
<ModalShell
	...
	description="Quem deve assinar esta escala?"
	largura="md"
	pending={enviandoSolicitacao}
		...
		{#if opcaoSolicitacao === 'respondencia'}
			<div class="pl-4 space-y-2 pt-1 animate-fade-in">
				...
				{:else}
					...
					{#if busca.buscando}
						<p
							class="text-xs text-surface-600 dark:text-surface-400 px-1 flex items-center gap-1.5"
							role="status"
							aria-live="polite"
						>
						...
						>
							{#each busca.resultados as p (p.id)}
								...
								>
									...
									<div class="text-xs text-surface-600 dark:text-surface-400">{p.lotacao}</div>
								</button>
							{/each}
						</div>
					{:else if busca.erro}
						<p class="text-xs text-surface-600 dark:text-surface-400 px-1">
							{busca.erro}
						</p>
					{/if}
				{/if}
