...
export function parseRespostasFormularioJsonLoose(raw: string | null | undefined): Record<string, unknown> {
...
export function parseRespostasFormularioJsonStrict(raw: string): { ok: true; data: GiseRespostasFormRecord } | { ok: false } {
	let parsed: unknown;
	try {
		parsed = JSON.parse(raw);
	} catch {
