 ...
 * PATCH /api/gise/[id]  → atualiza campos (hora_entrada, hora_saida, supervisores)
 * DELETE /api/gise/[id] → exclui (Admin Geral)
 */

import { json } from '@sveltejs/kit';
import type { RequestHandler } from './$types';
import { getDB, buscarGiseDetalhado, atualizarGiseEscala, buscarGiseEscala } from '$lib/db';
import { isAdminGeral } from '$lib/auth';
import { policiais, giseEscalas } from '$lib/server/schema';
...
export const PATCH: RequestHandler = async ({ locals, params, request, platform }) => {
	...
	const { hora_entrada, hora_saida, supervisor_sabado_id, supervisor_domingo_id } = body as {
		hora_entrada?: string;
		hora_saida?: string;
...
export const DELETE: RequestHandler = async ({ locals, params, platform }) => {
	...
	if (gise.status === 'finalizada') {
		return json({ error: 'Não é possível excluir uma escala finalizada' }, { status: 400 });
	}
