...
export const POST: RequestHandler = async ({ request, platform, getClientAddress }) => {
	try {
		...
		if (!usuario) {
			logger.info('[auth/redefinicao] Identificador não encontrado', { tipo, ip });
			return json({ message: RESPOSTA_GENERICA });
		}

		if (!usuario.email_pessoal) {
			logger.info('[auth/redefinicao] Usuário sem e-mail pessoal', {
				tipo,
		...
		if ((userCodeCount?.n ?? 0) >= MAX_CODIGOS_USUARIO) {
			logger.warn('[auth/redefinicao] Rate limit por usuário ao solicitar código', {
				tipo,
				usuario_id: usuario.id,
				ip
			});
		...
		try {
			...
			logger.error('[auth/redefinicao] Falha ao enviar código de redefinição', {
				tipo,
				usuario_id: usuario.id,
		...
		logger.error('[auth/redefinicao] Falha inesperada ao solicitar redefinição', {
			error: err instanceof Error ? err.message : String(err)
		});
		return json({ message: RESPOSTA_GENERICA });
	}
