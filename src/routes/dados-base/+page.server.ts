/**
 * `/dados-base` — onde a unidade informa a LINHA DE BASE de cada indicador.
 *
 ...
 *
 * Nomeado (em vez de inferido no `return`) porque o `load` tem uma saída curta
 * para "nenhuma operação ativa": sem o tipo declarado, os dois `return` produzem
 * uma união com `{}` e a indexação por número deixa de compilar na tela.
 */
...
export const load: PageServerLoad = async ({ locals, platform, url, depends }) => {
	...
	const operacao =
		operacoesLista.find((o) => o.id === operacaoIdParam) ?? operacoesLista[0] ?? null;
	...
	return {
		operacoes: operacoesLista,
		operacaoSelecionadaId: operacao.id,
		indicadores,
