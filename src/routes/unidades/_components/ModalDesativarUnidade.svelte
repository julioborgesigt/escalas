	...
	>
		...
		>
			<Dialog.Title class="h3 font-bold mb-2">
				{reativando ? 'Reativar unidade?' : 'Desativar unidade?'}
			</Dialog.Title>
			<Dialog.Description class="text-surface-600 dark:text-surface-400 mb-4">
				{#if reativando}
					"{unidade?.nome}" volta a aparecer nas listas de escolha (nova escala, lotação de
					servidor, seccional de GISE).
				{:else}
			...
			{#if !reativando}
				...
				>
					...
					<p>
						Escalas, lotações e assinaturas já existentes continuam funcionando normalmente, e a
						unidade pode ser reativada a qualquer momento.
					</p>
					<p>
						Unidade <strong>nunca é excluída</strong> do sistema: a linha é referenciada pelos registros
						de assinatura dos relatórios de GISE, e apagá-la destruiria a prova desses documentos.
					</p>
				</div>
			{/if}
