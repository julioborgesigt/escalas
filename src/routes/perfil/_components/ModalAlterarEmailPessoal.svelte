...
<ModalShell
	bind:open
	title={ehTroca ? 'Trocar e-mail pessoal' : 'Cadastrar e-mail pessoal'}
	description={verificacao.etapa === 'dados'
		? 'O e-mail pessoal é o seu canal de recuperação de senha.'
		: 'Digite o código de 6 dígitos enviado ao novo endereço.'}
	largura="md"
	pending={verificacao.enviando}
			...
			{#if ehTroca}
				<label class="label">
					...
					<input
						...
						maxlength="128"
					/>
					<span class="text-2xs text-surface-600 dark:text-surface-400 mt-1 block">
						Por segurança, a troca do canal de recuperação exige sua senha. Um aviso será enviado ao
						seu e-mail funcional.
					</span>
				</label>
			{/if}
		</div>
