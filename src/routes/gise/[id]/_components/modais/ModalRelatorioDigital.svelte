...
<svelte:window onkeydown={(e) => { if (open && e.key === 'Escape' && !disabled) onClose(); }} />
...
{#if open}
	...
	>
		<div
			class="bg-surface-50 dark:bg-surface-900 rounded-3xl shadow-2xl w-full max-w-lg p-5 sm:p-8 space-y-5 sm:space-y-6 border border-white/10 max-h-[calc(100dvh-1.5rem)] overflow-y-auto"
			role="dialog"
			aria-modal="true"
		>
