<script lang="ts">
	...
	const isUnidade = $derived(giseEstado.isUnidade);
	const isSupervisor = $derived(giseEstado.isSupervisor);
	const isMembro = $derived(giseEstado.isMembro);
												...
												>
													{#if isMobile || !data.restringirSmartphone}
														{@render actionButton(
															...
															'filled',
															() =>
																assinatura.abrirAssinaturaRelatorio(
																	sec.seccional_id,
																	'extraordinario'
																),
															undefined,
										...
										<div class="p-3 space-y-2.5">
											{#each slot.equipes ?? [] as equipe}
												...
												>
													...
													{:else}
														<p class="text-sm text-surface-400 italic mb-3">
