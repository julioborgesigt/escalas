<script lang="ts">
	...
	const delegaciasDaSeccional = $derived(
		unidades.filter(
			(u: Unidade) =>
				u.tipo === 'delegacia' && u.seccional_id === papelUnidadeId
		)
	);
	...
	// ========== Modal Nova Escala ==========
	const MESES_PT_NE = ['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'];
	...
	const mesAnteriorInfo = $derived.by(() => {
		...
		const existe = (data.escalasExistentes as any[]).some(
			(e) => e.lotacao === neLotacao && e.tipo === novaEscalaTipo && e.mes === mesPrev && e.ano === anoPrev
		);
	...
	function nextMesNE(): number { const m = new Date().getMonth() + 1; return m === 12 ? 1 : m + 1; }
	function nextAnoNE(): number { const h = new Date(); return h.getMonth() + 1 === 12 ? h.getFullYear() + 1 : h.getFullYear(); }
	function isoDiaLocalNE(year: number, month: number, day: number): string {
		return `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
	}
	function fmtDiaNE(iso: string): string { const [, m, d] = iso.split('-'); return `${d}/${m}`; }
	...
	function tiposDisponiveisNE(u: Unidade) {
		...
		if (u.tem_plantao) tipos.push({ tipo: 'plantao', label: 'Plantão Mensal', desc: `${MESES_PT_NE[nextMesNE() - 1]} ${nextAnoNE()}`, icon: '🌙' });
		if (u.tem_expediente) tipos.push({ tipo: 'expediente', label: 'Expediente Mensal', desc: `${MESES_PT_NE[nextMesNE() - 1]} ${nextAnoNE()}`, icon: '☀️' });
	...
	function escolherTipoNE(tipo: 'plantao' | 'expediente' | 'fds') {
		if (tipo === 'fds') {
			...
			calMes = sab.getMonth();
			fdsHoraEntrada = '08'; fdsMinutoEntrada = '00';
			fdsHoraSaida = '08'; fdsMinutoSaida = '00';
		} else {
			neDataInicio = '';
			neDataFim = '';
			neTitulo = '';
	...
	const fdsHorarioLabelNE = $derived(`${fdsHoraEntrada}:${fdsMinutoEntrada}H A ${fdsHoraSaida}:${fdsMinutoSaida}H`);
	...
	function calRemoverDiaNE(iso: string) { fdsDiasSelecionados = fdsDiasSelecionados.filter((d) => d !== iso); }
...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
	...
	<div class="flex items-center gap-2">
		...
		<button type="button" class="btn btn-sm preset-filled-primary-500" onclick={abrirNovaEscala}>Nova Escala</button>
	</div>
</div>
...
<Dialog open={dialogNovaEscalaAberto} onOpenChange={(e) => { if (!e.open) fecharNovaEscala(); }}>
	<Dialog.Content
		class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-surface-950/80 backdrop-blur-sm"
	>
		<div
		...
		>
			...
			{#if novaEscalaTipo === null}
				...
				<div class="flex justify-end mt-6">
					<button type="button" class="btn preset-outlined-surface" onclick={fecharNovaEscala}>Cancelar</button>
				</div>
				...
				<div class="space-y-2.5">
					...
					<div class="rounded-xl border border-surface-200 dark:border-surface-700 p-2 sm:p-2.5 space-y-1 bg-white dark:bg-surface-800/40">
						<div class="flex items-center justify-between gap-1.5">
							<button type="button" class="btn preset-outlined-surface-500 p-1.5 rounded-lg shrink-0" aria-label="Mês anterior" onclick={calMesAnteriorNE}>
							...
							<button type="button" class="btn preset-outlined-surface-500 p-1.5 rounded-lg shrink-0" aria-label="Próximo mês" onclick={calMesProximoNE}>
							...
							{#each gradeCalendarioNE as cell}
								{#if cell}
									...
									<button
										...
										aria-pressed={sel}
										aria-label="Dia {cell.day} de {MESES_PT_NE[calMes]}"
									>{cell.day}</button>
								{:else}
									<div class="h-9"></div>
								{/if}
					...
					{#if fdsDiasOrdenadosNE.length > 0}
						<div class="min-w-0 space-y-0.5">
							<span class="text-[0.65rem] font-semibold text-surface-500">Dias selecionados ({fdsDiasOrdenadosNE.length})</span>
							<div class="flex flex-nowrap items-stretch gap-1.5 overflow-x-auto max-w-full pb-0.5 [scrollbar-width:thin]">
								{#each fdsDiasOrdenadosNE as iso}
									<span class="inline-flex items-center gap-0.5 pl-1.5 pr-0.5 py-0.5 rounded-md text-[0.65rem] font-medium border shrink-0 border-warning-400/80 bg-warning-500/10 text-warning-900 dark:text-warning-100">
										{fmtDiaNE(iso)}
										<button type="button" class="p-0.5 rounded text-surface-400 hover:text-error-600 dark:hover:text-error-400 shrink-0" aria-label="Remover {fmtDiaNE(iso)}" onclick={() => calRemoverDiaNE(iso)}>
					...
					<div class="rounded-xl border border-surface-200 dark:border-surface-700 p-2.5 space-y-1.5">
						...
						<p class="text-[0.65rem] text-primary-600 dark:text-primary-400 font-medium">{fdsHorarioLabelNE}</p>
					</div>
					...
					{#if fdsTituloAutoNE}
						<div class="rounded-lg bg-surface-100 dark:bg-surface-800/50 px-3 py-2">
							...
							<p class="text-xs text-surface-700 dark:text-surface-200 font-medium leading-snug">{fdsTituloAutoNE}</p>
						</div>
					{/if}

					<div class="flex justify-end gap-2 pt-1">
						<button type="button" class="btn preset-outlined-surface text-xs sm:text-sm px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg sm:rounded-xl" onclick={() => (novaEscalaTipo = null)}>
							← Voltar
						</button>
			...
			{:else}
				...
				<div class="space-y-4">
					<div>
						<h2 class="text-base font-bold text-surface-900 dark:text-surface-50 leading-tight">
							Nova Escala — {novaEscalaTipo === 'plantao' ? 'Plantão Mensal' : 'Expediente Mensal'}
						</h2>
						{#if unidadeNovaEscala}
							<p class="text-xs text-surface-500 mt-0.5">{unidadeNovaEscala.nome}</p>
						{/if}
					...
					<div class="flex items-center justify-between gap-2">
						...
						<span class="font-bold text-lg text-surface-900 dark:text-surface-50">{nePickerAno}</span>
						<button
							type="button"
							class="btn preset-outlined-surface-500 p-1.5 rounded-lg"
					...
					<div class="grid grid-cols-4 gap-2">
						{#each MESES_PT_NE as nomeMes, i}
							...
							{@const selecionado = neMesSelecionado?.mes === mesNum && neMesSelecionado?.ano === nePickerAno}
							<button
								type="button"
								disabled={ocupado}
					...
					{#if neTitulo}
						<div class="rounded-lg bg-surface-100 dark:bg-surface-800/50 px-3 py-2">
							<p class="text-[0.6rem] text-surface-400 mb-0.5">Título gerado</p>
							<p class="text-xs text-surface-700 dark:text-surface-200 font-medium leading-snug">{neTitulo}</p>
						</div>
						...
						<div class="grid grid-cols-2 gap-2">
							...
							<form method="POST" action="?/criarComBase" use:enhance={handleCriarComBase} class="contents">
								...
								>
									<p class="font-semibold text-sm text-surface-800 dark:text-surface-100 leading-tight">
										Com base em {MESES_PT_NE[(mesAnteriorInfo?.mes ?? 1) - 1]}/{mesAnteriorInfo?.ano}
									</p>
							...
							<form method="POST" action="?/criar" use:enhance={handleCriar} class="contents">
								...
								<input type="hidden" name="hora_entrada" value={`${neHoraEntrada}:${neMinutoEntrada}`} />
								<input type="hidden" name="hora_saida" value={`${neHoraSaida}:${neMinutoSaida}`} />
								<input type="hidden" name="tipo" value={novaEscalaTipo} />
					...
					<div class="flex justify-start pt-1">
						<button
							...
							onclick={() => { novaEscalaTipo = null; neDataInicio = ''; neDataFim = ''; neTitulo = ''; }}
						>← Voltar</button>
					</div>
				</div>
			{/if}
...
>
	...
	>
		...
		<label class="label col-span-12 {isAdmin || isAdminSeccional ? 'sm:col-span-2' : 'sm:col-span-6'}">
		...
		<label class="label col-span-6 {isAdmin || isAdminSeccional ? 'sm:col-span-1' : 'sm:col-span-3'}">
	...
	{:else if escalas.length === 0}
		<div class="text-center py-12 text-surface-500">
			...
			<button type="button" class="btn preset-filled-primary-500" onclick={abrirNovaEscala}>Criar Escala</button>
		</div>
	{:else}
		<!-- Desktop: tabela -->
			...
			{#each escalas as esc (esc.id)}
				...
				>
					<div class="flex justify-between items-start mb-3 gap-2">
						...
						>
						{#if esc.is_assinada}
							<span class="badge preset-filled-success-500 font-bold px-1.5 py-0.5 text-[0.65rem] rounded flex items-center gap-1 shadow-sm">
						...
						{:else}
							<span class="badge preset-tonal-warning font-bold px-1.5 py-0.5 text-[0.65rem] rounded flex items-center gap-1 shadow-sm">
