...
<div class="space-y-4">
	{#snippet renderCampo(q: any, level = 0)}
		...
		<div class="card-indent-wrapper" style="--indent: {level * 1.5}rem">
			<div class="card p-4 sm:p-6 bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-800 rounded-3xl shadow-sm space-y-4 animate-in fade-in slide-in-from-top-4 duration-500">
				<div class="space-y-1">
				...
				<div class="mt-2">
					{#if q.tipo === 'vtr_placa'}
						<input
							...
							class="w-full px-4 py-3 rounded-xl border border-surface-300 dark:border-surface-600 bg-surface-50 dark:bg-surface-800 text-sm font-black uppercase tracking-widest focus:ring-2 focus:ring-primary-500 transition-all shadow-inner"
					...
					{:else if q.tipo === 'numero'}
						<input
							...
							type="number"
							placeholder="0"
							class="w-full sm:w-48 px-4 py-3 rounded-xl border border-surface-300 dark:border-surface-600 bg-surface-50 dark:bg-surface-800 text-sm font-bold focus:ring-2 focus:ring-primary-500 transition-all shadow-inner"
							bind:value={respostas[q.key]}
						/>
					{:else if q.tipo === 'select_99'}
						<select
							id="q-{q.id}"
							class="w-full sm:w-32 px-4 py-3 rounded-xl border border-surface-300 dark:border-surface-600 bg-surface-50 dark:bg-surface-800 text-sm font-bold focus:ring-2 focus:ring-primary-500 transition-all shadow-inner appearance-none"
							bind:value={respostas[q.key]}
						>
					...
					{:else if q.tipo === 'mandados_maiores' || q.tipo === 'prisoes_maiores' || q.tipo === 'apreensoes_menores'}
						<div class="space-y-4">
							...
							{#if respostas[q.key] === 'Sim'}
								<div class="p-3 sm:p-6 bg-surface-50 dark:bg-surface-950/40 rounded-2xl border border-surface-200 dark:border-surface-800 space-y-4 animate-in fade-in zoom-in-95 duration-500">
									<div class="flex items-center gap-4">
										<label class="block w-full sm:w-auto">
											<span class="text-[0.65rem] font-black text-surface-400 uppercase tracking-widest block mb-1">{q.subtexto_qtd || 'Quantidade:'}</span>
											<select class="w-full sm:w-24 px-4 py-2.5 rounded-xl border border-surface-300 dark:border-surface-600 bg-white dark:bg-surface-900 text-sm font-bold shadow-sm"
									...
									<div class="space-y-3">
										...
										{#each (respostas[resKey] || []) as item, i}
											<div class="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3 sm:p-4 bg-white dark:bg-surface-900 rounded-2xl border border-surface-100 dark:border-surface-800 shadow-sm transition-all hover:border-primary-500/30">
												<div class="space-y-1">
													<label class="text-[0.6rem] font-bold text-surface-400 uppercase" for="n-{q.id}-{i}">Nome</label>
													<input id="n-{q.id}-{i}" type="text" placeholder="Nome Completo" class="w-full px-3 py-2.5 rounded-lg border border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-950 text-sm font-medium"
												...
												<div class="space-y-1">
													...
													<input id="m-{q.id}-{i}" type="text" placeholder="Número" class="w-full px-3 py-2.5 rounded-lg border border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-950 text-sm font-medium"
							...
							{#if respostas[q.key] === 'Sim'}
								<div class="p-3 sm:p-6 bg-surface-50 dark:bg-surface-950/40 rounded-2xl border border-surface-200 dark:border-surface-800 space-y-4 animate-in fade-in zoom-in-95 duration-500">
									<div class="space-y-3">
										...
										<div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
											{#each ['Maconha', 'Cocaína', 'Crack', 'Extase', 'LSD', 'Haxixe', 'Skunk', 'Outros'] as d}
												<button class="px-3 py-2.5 rounded-xl text-xs font-black uppercase border-2 transition-all {(respostas.drogas_selecionadas || []).includes(d) ? 'bg-primary-500 text-white border-primary-500 shadow-md' : 'bg-white dark:bg-surface-900 text-surface-500 border-surface-100 dark:border-surface-800 hover:border-primary-500/50'}"
													onclick={() => {
									...
									{#if (respostas.drogas_selecionadas || []).length > 0}
										<div class="space-y-3 pt-2 border-t border-surface-100 dark:border-surface-800 transition-all">
											...
											{#each respostas.drogas_selecionadas as d}
												<div class="flex flex-col sm:flex-row sm:items-center gap-3 p-3 sm:p-4 bg-white dark:bg-surface-900 rounded-2xl border border-surface-100 dark:border-surface-800 shadow-sm animate-in slide-in-from-left-2 duration-300">
													<span class="text-sm font-black text-surface-600 dark:text-surface-400 uppercase tracking-tight">{d}:</span>
													...
													<div class="flex items-center gap-3 flex-1">
														<input
															...
															step="0.001"
															placeholder="0.000"
															class="flex-1 px-4 py-2.5 rounded-xl border border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-950 text-sm font-bold focus:ring-2 focus:ring-primary-500 transition-all shadow-inner"
														...
														<div class="flex p-1 bg-surface-100 dark:bg-surface-800 rounded-xl border border-surface-200 dark:border-surface-700">
															{#each ['g', 'kg'] as u}
																<button
																	type="button"
																	class="px-3 py-2 rounded-lg text-xs font-black uppercase transition-all {(respostas.drogas_unidade && respostas.drogas_unidade[d] === u) || (!respostas.drogas_unidade?.[d] && u === 'g') ? 'bg-white dark:bg-surface-700 text-primary-600 shadow-sm' : 'text-surface-400 hover:text-surface-600'}"
									...
									<div class="space-y-3">
										...
										<div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
											{#each ['Revolver', 'Pistola', 'Arma Longa', 'Arma Branca', 'Munição', 'Outros'] as a}
												<button class="px-3 py-2.5 rounded-xl text-xs font-black uppercase border-2 transition-all {(respostas.armas_selecionadas || []).includes(a) ? 'bg-primary-500 text-white border-primary-500 shadow-md' : 'bg-white dark:bg-surface-900 text-surface-500 border-surface-100 dark:border-surface-800 hover:border-primary-500/50'}"
													onclick={() => {
									...
									{#if (respostas.armas_selecionadas || []).length > 0}
										<div class="space-y-3 pt-2 border-t border-surface-100 dark:border-surface-800 transition-all">
											...
											{#each respostas.armas_selecionadas as a}
												<div class="flex items-center gap-4 p-3 sm:p-4 bg-white dark:bg-surface-900 rounded-2xl border border-surface-100 dark:border-surface-800 shadow-sm animate-in slide-in-from-left-2 duration-300">
													<span class="text-sm font-black flex-1 text-surface-600 dark:text-surface-400 uppercase tracking-tight">{a}:</span>

													<select class="w-28 px-4 py-2.5 rounded-xl border border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-950 text-sm font-bold focus:ring-2 focus:ring-primary-500 transition-all"
				...
				{#if (q.tipo === 'sim_nao' || q.tipo === 'mandados_maiores' || q.tipo === 'prisoes_maiores' || q.tipo === 'apreensoes_menores' || q.tipo === 'drogas_complex' || q.tipo === 'armas_complex') && respostas[q.key] === 'Sim' && q.filhos && q.filhos.length > 0}
					<div class="mt-4 space-y-4 pt-4 border-l-4 border-primary-500/20 pl-2 sm:pl-0">
