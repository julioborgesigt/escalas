...
- Abertas (ver status no cabeçalho de cada arquivo):
  ...
  - [`auditorias/AUDITORIA_COMPONENTIZACAO_2026-08-13.md`](auditorias/AUDITORIA_COMPONENTIZACAO_2026-08-13.md) — componentização e manutenibilidade; rodada seguinte à de 06/ago (arquivada). Lotes A–C remediados (§10): `isMobile` unificado, badge de tipo de escala com fonte única, `useResGise` dividido por audiência, regra do menu extraída com testes, actions de `escalas/[id]` em `_actions/`, `db/audit` e `export/pdf` fatiados. **Abertos:** §3.1 (prop drilling de 38 props na supervisão GISE — como retomar em §11) e o lote D de polimento.
